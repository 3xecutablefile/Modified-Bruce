...
static void bootTickIfNeeded() {
  ...
  if (sBootUsingLoRa) {
    switch (sBootLines % 4) {
      case 0: msg = String("[") + sp + "] root@apex:/# lora init @" + String((long)(sLoRaFreq/1000000)) + "MHz"; break;
      case 1: msg = String("[") + sp + "] root@apex:/# lora set sf=7 bw=125k crc=on"; break;
      case 2: msg = String("[") + sp + "] root@apex:/# chatd start --mode=lora"; break;
      default:msg = String("[") + sp + "] root@apex:/# peers listen"; break;
    }
  } else {
    switch (sBootLines % 4) {
      case 0: msg = String("[") + sp + "] root@apex:/# espnow init"; break;
      case 1: msg = String("[") + sp + "] root@apex:/# chat bind --iface wlan0"; break;
      case 2: msg = String("[") + sp + "] root@apex:/# peers discover --timeout=150ms"; break;
      default:msg = String("[") + sp + "] root@apex:/# chatd start --mode=broadcast"; break;
    }
  }
  sBootLines++;
  addTermWrapped(msg);
...
void infiChatDrawScreen(TFT_eSPI& tft) {
  if (!sInit) {
    geometrySetup(tft);
    drawBackground(tft);
    drawHeader(tft, "apex terminal — /root", "");
    renderPickToTerminal();
    sInit = true;
  }
  loRaPollRx();
  ...
  bootTickIfNeeded();
  switch (sMode) {
    case MODE_PICK:       drawHeader(tft, "apex terminal — /root", "pick"); break;
    case MODE_LORA_BAND:  drawHeader(tft, "apex terminal — /lora", "region"); break;
    case MODE_BOOT:       drawHeader(tft, sBootUsingLoRa ? "apex terminal — /lora" : "apex terminal — /chat", "starting…"); break;
    case MODE_CHAT:       drawHeader(tft, sTransport==TR_LORA ? "apex terminal — /lora" : "apex terminal — /chat", sShowKbd ? "typing…" : ""); break;
  }
...
void infiChatHandleInput(bool a, bool b, bool c, bool& exitReq) {
  ...
  if (sMode == MODE_PICK) {
    ...
    if (a) {
      if (sPickIdx == 0) { 
        ...
        addTermRaw("root@apex:/# starting chat …");
        sHeaderDirty = sTermDirty = true;
      } else {             
  ...
  if (sMode == MODE_LORA_BAND) {
    ...
    if (a) {
      ...
      sLoRaReady = false; sLoRaTriedInit = false; 
      sMode = MODE_BOOT; sLastTick = 0; sBootLines = 0; sTermCount = 0;
      addTermRaw("root@apex:/# starting LoRa …");
      sHeaderDirty = sTermDirty = true;
      return;
    }
  }
