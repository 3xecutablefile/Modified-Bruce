...
static void termDrawHeader(const char* title) {
  ...
  tft.print(title ? title : "apex terminal — /secure");
...
static void termTick_likeScan(const char* headerTitle) {
  ...
  String msg;
  switch ((sTermPhase++) % 4) {
case 0: msg = "["+String(spc)+"] root@apex:/# apply --stage=prepare"; break;
case 1: msg = "["+String(spc)+"] root@apex:/# write --secure-config"; break;
case 2: msg = "["+String(spc)+"] root@apex:/# verify --checksum"; break;
default: msg = "["+String(spc)+"] root@apex:/# finalize"; break;
  }
...
static bool uiGetNumeric(char outDigits[5]) {
  ...
  int pos = 0, cur = 0;
  drawTitleAndTerminal("apex terminal — /secure");
  while (pos < 4) {
...
bool Core::Passlock::runSettingsFlow() {
  if (g_enabled) {
    ...
    if (act == 0) {
      ...
      g_pattern.clear();
      saveState();
      terminalAndReboot("apex terminal — /secure");
      return true; 
    }
